...
type App struct {
	...
	searching            bool
	addingWhitelist      bool
	addingMapLocalInput  bool
	managingWhitelist    bool
	managingMapLocal     bool
	addingMapLocal       bool
	managingMapRemote    bool
	addingMapRemote      bool
	lastKeyRune          rune
...
func (app *App) isPopupOpen() bool {
	return app.searching || app.addingWhitelist || app.addingMapLocalInput ||
		app.managingWhitelist || app.managingMapLocal || app.addingMapLocal ||
		app.managingMapRemote || app.addingMapRemote || app.whichKey.IsVisible()
}
...
func (app *App) getCurrentPopupPrimitive() tview.Primitive {
	...
	if app.addingWhitelist {
		return app.whitelistInput
	}
	if app.addingMapLocalInput {
		return app.mapLocalPatternInput
	}
	if app.managingWhitelist {
		return app.whitelistManager
	}
	if app.managingMapLocal {
		return app.mapLocalManager
	}
	if app.addingMapLocal {
		return app.mapLocalForm
	}
	if app.managingMapRemote {
		return app.mapRemoteManager
	}
	if app.addingMapRemote {
...
func (app *App) handleInput(event *tcell.EventKey) *tcell.EventKey {
	...
	if app.searching || app.addingWhitelist || app.addingMapLocalInput || app.managingWhitelist || app.managingMapLocal || app.addingMapLocal || app.managingMapRemote || app.addingMapRemote {
		return event
	}
...
func (app *App) quickMapLocal() {
	...
	if err := os.MkdirAll(mappingsDir, 0o755); err != nil {
		app.layout.SetStatus(fmt.Sprintf("[red]Failed to create mappings dir: %s[-]", err))
		return
	}

	// Generate filename from URL (always .jsonc)
	filename := generateFilename(flow.Request.URL) + ".jsonc"
	localPath := filepath.Join(mappingsDir, filename)

	// Build JSONC response format
	var buf bytes.Buffer

	buf.WriteString("{\n")
	buf.WriteString(fmt.Sprintf("  // Mapped from: %s\n", flow.Request.URL))
	buf.WriteString(fmt.Sprintf("  // Generated: %s\n", time.Now().Format(time.RFC3339)))
	buf.WriteString("\n")

	// Status
	buf.WriteString(fmt.Sprintf("  \"status\": %d,\n", flow.Response.StatusCode))
	buf.WriteString(fmt.Sprintf("  \"statusText\": %q,\n", strings.TrimPrefix(flow.Response.Status, fmt.Sprintf("%d ", flow.Response.StatusCode))))
	buf.WriteString("\n")
	...
	// If body is JSON, embed it directly as an object
	if strings.Contains(contentType, "json") || isJSONContent(body) {
	...
	if err := os.WriteFile(localPath, buf.Bytes(), 0o644); err != nil {
		app.layout.SetStatus(fmt.Sprintf("[red]Failed to write file: %s[-]", err))
		return
	}

	// Create the URL pattern (exact match)
	pattern := flow.Request.URL
...
func (app *App) createMapLocalWithPattern(pattern string) {
	...
	// Generate filename from pattern (always .jsonc)
	filename := generateFilename(pattern) + ".jsonc"
	localPath := filepath.Join(mappingsDir, filename)
	...
	// Add the mapping with custom pattern
	app.viewModel.AddMapLocalRule(pattern, localPath, flow.Response.StatusCode, flow.Response.Headers.Get("Content-Type"))
...
// isJSONContent checks if the body looks like JSON
